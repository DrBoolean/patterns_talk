describe("Utils", function() {
  ...
  xit("compose (binary functions) *coming soon", function() {
    var doubleMap = compose(map, map);
    expect(doubleMap(toUpperCase, [["a"]])).toEqual([["A"]]);
  });

  it("compose (several functions)", function() {
    var doubleMap = compose(toUpperCase, charAt(1), dot(0));
    expect(doubleMap(["abc"])).toEqual("B");
  });

  it("compose (binary then unary function)", function() {
    var mapThenReverse = compose(reverse, map(toUpperCase));
    expect(mapThenReverse(["a", "b", "c"])).toEqual(["C", "B", "A"]);
  });
